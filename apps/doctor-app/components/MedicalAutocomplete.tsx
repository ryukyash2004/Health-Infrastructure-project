...
export default function MedicalAutocomplete({
  ...
  const fetchSuggestions = async (searchTerm: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`http://localhost:8000/api/v1/clinical/search?q=${encodeURIComponent(searchTerm)}&type=${type}`);
    ...
    <div ref={containerRef} className={`relative w-full ${className}`}>
      <div className="relative">
        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          <Search className={`h-4 w-4 ${isLoading ? 'text-blue-500' : 'text-slate-400'}`} />
        </div>
        ...
        <input
          ...
          className="block w-full pl-9 pr-8 py-2 border border-slate-200 rounded-md bg-white text-sm placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700"
        />
